...
var AwsClient = class {
  ...
  async fetch(input, init) {
    for (let i4 = 0; i4 <= this.retries; i4++) {
      ...
      await new Promise((resolve) => setTimeout(resolve, Math.random() * this.initRetryMs * Math.pow(2, i4)));
    }
...
var AwsV4Signer = class {
  constructor({ method, url, headers, body, accessKeyId, secretAccessKey, sessionToken, service, region, cache, datetime, signQuery, appendSessionToken, allHeaders, singleEncode }) {
    ...
    this.encodedSearch = [...this.url.searchParams].filter(([k4]) => {
      if (!k4)
        return false;
      if (this.service === "s3") {
        if (seenKeys.has(k4))
      ...
      return true;
    }).map((pair) => pair.map((p4) => encodeRfc3986(encodeURIComponent(p4)))).sort(([k1, v1], [k22, v2]) => k1 < k22 ? -1 : k1 > k22 ? 1 : v1 < v2 ? -1 : v1 > v2 ? 1 : 0).map((pair) => pair.join("=")).join("&");
  }
  async sign() {
    if (this.signQuery) {
...
function encodeRfc3986(urlEncodedStr) {
  return urlEncodedStr.replace(/[!'()*]/g, (c4) => "%" + c4.charCodeAt(0).toString(16).toUpperCase());
}
...
var streamCollector = (stream2) => new Promise((resolve, reject) => {
...
async function getObject(url) {
  ...
  if (response.status === 200) {
    return {
      // response.body type is wrong until we fix our fetch/Response polyfill
      body: response.body,
...
var ServerlessResponse = class extends import_node_http.default.ServerResponse {
  ...
  constructor(request2) {
    super(request2);
    const body = new import_node_stream.Readable({ read() {
    } });
    this[BODY] = body;
    this[HEADERS] = {};
  ...
  async readStaticFile(relativePath) {
    ...
    if (this.useNode) {
      if (result && result instanceof ReadableStream) {
        const nodeStream = import_node_stream2.default.Readable.fromWeb(result);
        return nodeStream;
      } else {
        return result;
      }
